...
class BEVRender:
    ...
    def render(
        ...
        for i in range(0,6):
            ...
            self._render_command(data)
            self._render_legend()
            self.draw_planning_json(result, i)
            save_path_pred = os.path.join(self.pred_dir, str(index).zfill(4) + f'_{i}.jpg')
    ...
    def draw_detection_gt(self, data):
        ...
        for i in range(data['gt_names'].shape[0]):
            label = data['gt_names'][i]
            if label == "traffic_cone": 
                continue
    ...
    def draw_motion_gt(self, data):
        ...
        for i in range(data['gt_names'].shape[0]):
            ...
            center = data['gt_boxes'][i, :2]
            masks = data['gt_agent_fut_masks'][i].astype(bool)
            if masks[0] == 0:
                continue

            # if data['gt_agent_fut_trajs'].shape[-1] !=6:
            #     trajs = data['gt_agent_fut_trajs'][i].reshape(6,2)[masks]
            # else:
    ...
    def draw_planning_json(self, result, idx):
        if not self.plot_choices['planning']:
            return
        if idx == 0:
            plan_traj = np.array(result['base_traj']).reshape(6,2)
        else:
            plan_traj = np.array(result[f'traj_pred_{idx-1}']).reshape(6,2)
        plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
        self._render_traj(plan_traj, traj_score=1.0,
            colormap='autumn', dot_size=50)
    ...
    def _render_sdc_car(self):
        sdc_car_png = cv2.imread('./sdc_car.png')
